...
var audioEventHandlers = Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
    ...
    'PlaybackNearlyFinished' : function () {
        ...
        this.attributes['enqueuedToken'] = String(this.attributes['playOrder'][enqueueIndex].token);

        var enqueueToken = this.attributes['enqueuedToken'];
        var playBehavior = 'ENQUEUE';
        var podcast = this.attributes['playOrder'][enqueueIndex];
...
function getIndex() {
    ...
    for(var i = 0, len = playOrder.length; i < len; i++) {
        if (playOrder[i].token === tokenValue) {
            return i;
        }
    }
