...
var fetchAudioData = require('./audioAssets');
var constants = require('./constants');

var welcomeMsg = 'Welcome to the NPR Weekend Edition Podcast. You can say, play saturday or play sunday to begin the podcast.';
var welcomeRepromptMsg = 'You can say, play saturday or play sunday, to begin.';
var goodByeMsg = 'Good bye.';

var launchRequest = function () {
    this.handler.state = constants.states.START_MODE;
    var message = welcomeMsg;
    var reprompt = welcomeRepromptMsg;
...
var initModeHandler = {
    ...
    'Unhandled' : function () {
        var message = 'Sorry, I could not understand. Please say, play saturday or play sunday, to begin the audio.';
        this.response.speak(message).listen(message);
        this.emit(':responseReady');
    }
};

// clone because CreateStateHandler requires separate obj
var startModeHandler = Object.assign({}, initModeHandler);

var stateHandlers = {
    // need to have empty mode since initially state is empty string, and if START_MODE
    // is set as empty string value then can't change state back to START_MODE, bug in SDK???
    initModeInitHandlers: Alexa.CreateStateHandler('', initModeHandler),

    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, startModeHandler),
    ...
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            if (this.attributes['playbackFinished']) {
                this.handler.state = constants.states.START_MODE;
                message = welcomeMsg;
                reprompt = welcomeRepromptMsg;
            } else {
                this.handler.state = constants.states.RESUME_DECISION_MODE;
                message = 'You were listening to ' + this.attributes['playOrder'][this.attributes['index']].title +
        ...
        'AMAZON.HelpIntent' : function () {
            ...
            var message = 'You are listening to the NPR Weekend Edition Podcast. You can say, Next or Previous to navigate through the playlist. ' +
...
var controller = function () {
    return {
        play: function () {
            ...
            var token = String(this.attributes['playOrder'][this.attributes['index']].token);
            var playBehavior = 'REPLACE_ALL';
            var podcast = this.attributes['playOrder'][this.attributes['index']];
        ...
        playNext: function () {
            ...
            if (index === this.attributes['playOrder'].length) {
        ...
        playPrevious: function () {
            ...
            if (index === -1) {
                if (this.attributes['loop']) {
                    index = this.attributes['playOrder'].length - 1;
                } else {
                    // Reached at the end. Thus reset state to start mode and stop playing.
                    this.handler.state = constants.states.START_MODE;
